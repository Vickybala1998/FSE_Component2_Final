...
namespace com.tweetapp.Controllers
{
    ...
    public class TweetController : Controller
    {
        ...
        public TweetController(ITweetService Service,ILogger<TweetController> Log,IOptions<DbSettings> dbSettings)
        {
            _service = Service;
            _log = Log;
        ...
        public IActionResult postTweet([FromBody] tweet _tweet,string username)
        {
            try
            {
                ...
                var sender = new com.tweetapp.ServiceBus.serviceBusSender(_dbSettings);
                sender.sendMessage(_tweet);
                if (_service.postTweet(_tweet))
                {
                    _log.LogInformation("tweet post completed");
        ...
        //{
        //    _log.LogInformation("tweet post started");
        //    if (_service.postTweet(_tweet))
        //    {
        //        _log.LogInformation("tweet post completed");
        //        return StatusCode(200);
